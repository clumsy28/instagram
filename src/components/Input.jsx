...
export default function Input({ label, type = 'text', ...props }) {
  ...
  return (
    <label className='relative flex bg-zinc-50 border rounded-[3px] focus-within:border-neutral-400 border-[#DBDBDB]'>
      <input type={inputType} className={classNames({
        'px-2 outline-none text-xs bg-transparent w-full h-[38px]': true,
        'pt-[10px]': field.value
      })} {...field} {...props} />
      <small className={classNames({
        'absolute left-[9px] cursor-text pointer-events-none text-neutral-500 -translate-y-1/2 transition-all': true,
